...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      //type区分用户点击的是哪一个按钮 disNum获取数值 cart知道是哪一个产品
      switch (type) {
        case "add":
          disNum + 1
          break;
    ...
    updateChecked(cart, event) {
      try {
        //因为event的checked的属性的返回值是布尔值 而我们要的数据是1或0 所以我们给它处理一下
    ...
    async deleteAllCheckedCart() {
      ...
      try {
        //由于在结构中拿不到上面的数据 所以得到vuex中用上下文对象获取数据并删除单个的action接口
