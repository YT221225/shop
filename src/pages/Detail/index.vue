...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      //如果服务器的数据没有回来 那么skuInfo的属性值就是undefined 那么就不会在页面假报错
  ...
  methods: {
    ...
    changeActive(spuSaleAttrValue, arr) {
      //遍历全部的售卖属性值 让所有的售卖属性值isChecked都为0
      arr.forEach(item => {
        item.isChecked = 0
      });
    ...
    //添加到购物车 进行路由跳转之前 发送请求把你购买的产品的信息通过请求的方式通知服务器 服务器进行存储
    async addshopcar(){
      //派发action 1.发请求 将产品加入到数据库(通知服务器) 2.服务器存储成功 携带参数进行路由跳转 3.失败给用户提醒 
      try {
        //该函数返回的是一个Promise的对象 那么我们需要等待它成功发或者失败的结果进行处理
        await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
        //我们需要将参数传递给下一级组件 我们采用query参数传递可以 但是有更好的写法(会话存储) 只携带一个参数个数 但是在这里我们要把对象转为json格式
