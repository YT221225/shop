<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input type="text" placeholder="请输入你的密码" v-model="password" name="password"
          v-validate="{ required: true, regex: /^[0-9A-Za-z]{8,20}$/ }" :class="{ invalid: errors.has('password') }" />
        <span class="error-msg">{{ errors.first('password') }}</span>
      </div>
      <div class="content">
        <label>确认密码:</label>
        <input type="text" placeholder="请再次输入你的密码" v-model="is_password" name="is_password"
          v-validate="{ required: true, is: password }" :class="{ invalid: errors.has('is_password') }" />
        <span class="error-msg">{{ errors.first('is_password') }}</span>
      </div>
      <div class="controls">
...
export default {
  ...
  methods: {
    ...
    //用户注册 在这里没有做任何判断 只是注册
    async userRegister() {
      const success = await this.$validator.validateAll()
      //保证所有的表单验证成功 在向服务器发送请求
      if (success) {
        try {
          const { phone, code, password, is_password, agree } = this
